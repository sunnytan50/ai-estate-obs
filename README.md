...
The Cloud Tokens row has three dropdowns — **Provider**, **Model**, **Token kind** — that filter every panel in the row (All = unfiltered). Pick one model (say `gpt-6-astra`) to get its daily tokens, daily cost, month-to-date figures, and its row in the 30-day breakdown table (tokens, output tokens, cost; column headers are filterable). The two per-model daily panels keep the 12 largest series via MetricsQL `topk_max()` (VictoriaMetrics-specific): Prometheus `topk()` re-ranks at every step, which makes stacked daily bars flicker. The dropdowns list whatever was used in the last 30 days regardless of the dashboard's time range — they use `query_result()` with a fixed lookback, not `label_values()`, which Grafana scopes to the visible range.
